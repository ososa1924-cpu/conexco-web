// ============================================================
// CONFIGURACION GENERAL DE CONEXCO
// ------------------------------------------------------------
// Cambia aqui los enlaces reales cuando los tengas disponibles.
// No necesitas tocar ningun otro archivo del proyecto.
// ============================================================

export const config = {
  // Enlace de descarga del APK de CONEXCO TIENDA
  storeDownloadUrl: "https://github.com/ososa1924-cpu/conexco-apk/releases/download/v2.8.2/conexco-tienda-v2.8.2.apk",

  // Enlace de descarga de CONEXCO TIENDA para Windows (PC)
  storeWindowsUrl: "https://github.com/ososa1924-cpu/conexco-apk/releases/download/v2.8.2/conexco-pc-v2.8.2-setup.zip",

  // Enlace de descarga del APK de CONEXCO CLIENTE
  clientDownloadUrl: "https://github.com/ososa1924-cpu/conexco-apk/releases/download/v2.8.2/conexco-cliente-v2.8.2.apk",

  // Numero de contacto (se muestra en la seccion de Soporte)
  supportPhone: "0424-237.7576",

  // Enlace de WhatsApp de soporte
  whatsappUrl: "https://wa.me/584242377576",

  // Correo de soporte
  supportEmail: "mailto:soporte@conexco.app",
};

export default config;